...
<div :style='{"width":"80%","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="售水机号" prop="shoushuijihao">
            <el-input v-model="ruleForm.shoushuijihao" 
                placeholder="售水机号" clearable :disabled=" false  ||ro.shoushuijihao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="库存" prop="kucun">
            <el-input v-model.number="ruleForm.kucun" 
                placeholder="库存" clearable :disabled=" false  ||ro.kucun"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="位置" prop="weizhi">
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          shoushuijihao: [
            { required: true, message: '售水机号不能为空', trigger: 'blur' },
          ],
          shangpinmingcheng: [
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='shoushuijihao'){
              this.ruleForm.shoushuijihao = obj[o];
              this.ro.shoushuijihao = true;
              continue;
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shoushuiji/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shoushuiji/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shoushuiji/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
