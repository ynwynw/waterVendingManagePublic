...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('dingdanxinxi','首页总数')">
		...
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('shangpinbuhuo','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{shangpinbuhuoCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>商品补货总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type4" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="dingdanxinxiChart1" class="echarts1" v-if="isAuth('dingdanxinxi','首页统计')"></div>
		<div id="dingdanxinxiChart2" class="echarts2" v-if="isAuth('dingdanxinxi','首页统计')"></div>
		<div id="dingdanxinxiChart3" class="echarts3" v-if="isAuth('dingdanxinxi','首页统计')"></div>
		<div id="shangpinbuhuoChart1" class="echarts4" v-if="isAuth('shangpinbuhuo','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getshangpinbuhuoCount();
		this.shangpinbuhuoChat1();
	},
	methods:{
		// 词云
        ...
        this.$http({
            url: `dingdanxinxi/value/shangpinmingcheng/kucun`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    dingdanxinxiChat2() {
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "dingdanxinxi/group/shangpinfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    dingdanxinxiChat3() {
      this.$nextTick(()=>{
        ...
        this.$http({
            url: `dingdanxinxi/value/xiadanshijian/zongjiage/月`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
		...
		getshangpinbuhuoCount() {
			this.$http({
				url: `shangpinbuhuo/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shangpinbuhuoCount = data.data
				}
			})
		},
		shangpinbuhuoChat1() {
        ...
        var shangpinbuhuoChart1 = echarts.init(document.getElementById("shangpinbuhuoChart1"),'macarons');
        this.$http({
            url: `shangpinbuhuo/value/shoushuijihao/kucun`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].shoushuijihao);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].shoushuijihao
                    })
                }
                var option = {};
				let titleObj = this.funnel.title
				titleObj.text = '补货数量统计'
                ...
                // 使用刚指定的配置项和数据显示图表。
                shangpinbuhuoChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shangpinbuhuoChart1.resize();
                };
            }
        });
      })
